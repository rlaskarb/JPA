...
import com.ohgiraffers.section01.entitymanager.EntityMangerGeneratorTest;
...
public class LifeCycleTest {
    ...
    @ParameterizedTest
    @DisplayName("비영속 테스트 (관리 하지 않을 때)")
    ...
    void testSameManager(int menuCode ){
        ...
        *  하게 된다면 영속성 컨텍스트가 관리할 수 있게 되며, 이를 영속 상태라고 한다.
        *  find(), createQuery() 을 사용한 조회도 자동으로 영속 상태가 된다.
        *  영속 상태인 엔티티 객체는 PK 로 조회를 하면 이미 관리가 되고 있기 때문에 같은 객체(인스턴스) 를 반환하게 된다. */
    }
    ...
    @ParameterizedTest
    @DisplayName("준 영속화 detach 후 재 영속화 (merge) 테스트")
    @CsvSource({"11,1000","12,1000"})
    void testDetachAndPersist(int menuCode , int menuPrice){
        ...
        *  만약 1차 캐시에 엔티티가 없으면 DB 에서 엔티티 조회 후 1차 캐시에 저장한다.
        *  조회한 영속 엔티티 객체에 준영속 상태의 엔티티 객체의 값을 병합 (merge)
        *  한뒤 영속 엔티티 객체를 반환하게 된다.*/
    ...
    @ParameterizedTest
    @DisplayName("영속성 엔티티 삭제  remove 해보기")
    @ValueSource(ints = {1})
    void testRemoveEntity(int menuCode){
        ...
        /*comment
        *  remove() 엔티티를 영속성 컨텍스트 및 데이터 베이스에서 제거한다
        *  단 트렌젝션을 제어하지 않으면 데이터 베이스에 영구 반영되지는 않는다.
        *  트렌잭션을 커밋 ㅡ 플러쉬 하는 순간 영속성 컨텍스트에서 관리하는
        *  엔티티 객체를 DB에 반영을 하게 된다.
        * */
